...
abstract class MY_Model extends CI_Model
{
    ...
    public function save()
    {
        ...
        if (!$this->{$this->primaryKey} || $this->{$this->primaryKey} === null)
        {
            // insert
            //echo("insert");
        ...
        else
        {
            ...
            // update
            //echo("update");
            $this->db->where(array($this->primaryKey => (int)$this->{$this->primaryKey}));
            $result = $this->db->update($this->table);
            $saved_object = !$result ? false : $this->getByID($this->{$this->primaryKey});
        }
    ...
    {
        $res = $this->db->delete($this->table, array($this->primaryKey => $this->{$this->primaryKey}));
        return !!$res;
    }
    ...
    {
        return $this->findByAttributes(array($this->primaryKey => $id));
    }
    ...
    {
        $query = $this->db->query('select max('.$this->primaryKey.') as id from ' . $this->table);
        return $query->row()->id;
    }
    ...
    private function get_save_array()
    {
        ...
        $exclude_fields = array('id', 'table', 'primaryKey');
